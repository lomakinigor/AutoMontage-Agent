...
[странице Pexels API](https://www.pexels.com/api/), затем сохрани его в локальном `.env` в корне
установки движка. Образец полей находится в `.env.example`; настоящий `.env` не коммить и ключ
не присылай в чат. Сервер читает ключ локально; браузер его не получает.
